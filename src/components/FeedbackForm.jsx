  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label 
          htmlFor="rating" 
          className="block text-sm font-medium text-gray-700 mb-1 accessible-label"
        >
        ...
        <div className="flex items-center" role="radiogroup" aria-labelledby="rating-group">
          ...
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              className={`h-8 w-8 ${rating >= star ? 'text-yellow-400' : 'text-gray-300'} accessible-focus`}
              aria-label={`${star} star${star !== 1 ? 's' : ''}, ${getRatingText(star)}`}
      ...
      <div>
        ...
        <textarea
          ...
          name="comment"
          rows={3}
          className="shadow-sm block w-full sm:text-sm border-gray-300 rounded-md accessible-input accessible-focus"
          placeholder="Tell us what you liked or how we can improve..."
          value={comment}
          onChange={(e) => setComment(e.target.value)}
          aria-describedby="comment-description"
        />
        <p id="comment-description" className="mt-1 text-sm text-gray-500 accessible-description">
      ...
      <div>
        ...
        <input
          ...
          autoComplete="email"
        />
        <p id="email-description" className="mt-1 text-sm text-gray-500 accessible-description">
      ...
      <div className="flex justify-end">
        <button
          type="button"
          onClick={onClose}
          className="mr-3 px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 accessible-focus"
        >
          Skip
        </button>
        <button
          type="submit"
          className="px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 accessible-focus"
        >
          Submit Feedback
        </button>
      </div>
